...
const imageFormatters: Record<string, undefined | t.ImageFormatter> = {
  ...
  // }),
  default: (item) => {
    let url = item.data.startsWith('http') ? item.data : `data:${item.mimeType};base64,${item.data}`;
    
    // Proxy fal.media URLs to avoid CORS issues
    if (url.startsWith('http') && (url.includes('fal.media') || url.includes('cdn.fal.ai') || url.includes('files.fal.ai'))) {
      const encodedUrl = encodeURIComponent(url);
      url = `/api/proxy-images?url=${encodedUrl}`;
    }
