...
function MCPSelect({
  ...
  useEffect(() => {
    ...
    if ((mcpServerSet?.size ?? 0) > 0) {
      const filteredValues = mcpValues.filter((mcp) => mcpServerSet?.has(mcp));
      // If no existing selections and "fal" is available, default to "fal"
      if (filteredValues.length === 0 && mcpServerSet?.has('fal')) {
        setMCPValues(['fal']);
        return;
      }
  ...
  return (
    <div style={hidden ? { position: 'absolute', left: '-9999px', visibility: 'hidden' } : {}}>
      <MultiSelect
        items={mcpServers ?? []}
