...
// Global credit adjustment factor - smaller numbers make credits go further
// For example: 0.1 = credits are 10x more valuable, 0.05 = credits are 20x more valuable
const CREDIT_ADJUSTMENT_FACTOR = 0.001; // Make credits 1000x more valuable (1000 credits = $1)
...
const getMultiplier = ({ valueKey, tokenType, model, endpoint, endpointTokenConfig }) => {
  ...
  if (endpointTokenConfig) {
    multiplier = endpointTokenConfig?.[model]?.[tokenType] ?? defaultRate;
  } else if (valueKey && tokenType) {
    multiplier = tokenValues[valueKey][tokenType] ?? defaultRate;
  } else if (!tokenType || !model) {
    multiplier = 1;
  } else {
  ...
  // Apply credit adjustment factor to make credits go further
  return multiplier * CREDIT_ADJUSTMENT_FACTOR;
};
...
const getCacheMultiplier = ({ valueKey, cacheType, model, endpoint, endpointTokenConfig }) => {
  ...
  if (endpointTokenConfig) {
    multiplier = endpointTokenConfig?.[model]?.[cacheType] ?? null;
  } else if (valueKey && cacheType) {
    multiplier = cacheTokenValues[valueKey]?.[cacheType] ?? null;
  } else if (!cacheType || !model) {
    multiplier = null;
  } else {
    valueKey = getValueKey(model, endpoint);
    if (!valueKey) {
  ...
  // Apply credit adjustment factor if multiplier exists
  return multiplier !== null ? multiplier * CREDIT_ADJUSTMENT_FACTOR : null;
};
